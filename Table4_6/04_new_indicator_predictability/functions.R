...
cleanup_new_indicator_outputs <- function(config) {
  if (!isTRUE(config$overwrite_new_indicator_outputs)) {
    return(invisible(NULL))
  }

  unlink(c(
    config$new_indicator_analysis_file,
    config$new_indicator_summary_file,
    config$new_indicator_comparison_file,
    config$new_indicator_predictability_rds
  ))

  existing_tables <- list.files(
    config$output_dir,
    pattern = "^new_indicator_regression_future_.*\\.csv$",
    full.names = TRUE
  )
  unlink(existing_tables)

  invisible(NULL)
}


# Auto-split Table 4 module: 20_new_indicator_predictability.R
...
new_indicator_period_key <- function(data, frequency) {
  ...
  stop(
    "NewIndicator file must contain YearMonth, YearWeek, Period, ",
    "FrequencyID, Week, or Date."
  )
}
...
load_new_indicator_ratios <- function(config) {
  ...
  if (!file.exists(path)) {
    warning(
      "NewIndicator source file is missing for ", config$frequency, ": ", path,
  ...
  # Identify indicator columns by the *_HL_Ratio naming convention.
  ...
  # Keep only the period key and ratio columns.
  out  <- data[, c("Period", ratio_cols), drop = FALSE]

  # Remove rows with a missing or empty period key.
  out  <- out[!is.na(out$Period) & nzchar(out$Period), , drop = FALSE]

  # Keep only the first occurrence of each period to avoid duplicate-key
...
run_new_indicators_predictability <- function(config, enriched) {
  # Skip the module entirely if disabled in the configuration.
  ...
  if (!isTRUE(config$overwrite_new_indicator_outputs) &&
      file.exists(config$new_indicator_predictability_rds) &&
      file.exists(config$new_indicator_summary_file)) {
    return(readRDS(config$new_indicator_predictability_rds))
  }
  cleanup_new_indicator_outputs(config)

  # Load indicator ratios; return empty result if the file is unavailable.
  indicators <- load_new_indicator_ratios(config)
  if (is.null(indicators)) {
    return(list(summary = data.frame(), models = list(), data = enriched))
  }

  # Ensure the enriched dataset has a Period column before merging.
  enriched$Period <- normalize_period_key(enriched, config$frequency)

  # Inner join: keep only periods present in both the enriched and indicator
  # datasets.
  merged <- merge(enriched, indicators, by = "Period", all = FALSE)
  merged <- merged[order(merged$Date), , drop = FALSE]

  # Identify the ratio columns that survived the merge.
  ratio_cols <- setdiff(names(indicators), "Period")
  ratio_cols <- intersect(ratio_cols, names(merged))
  if (!is.null(config$selected_new_indicator_predictors) &&
      length(config$selected_new_indicator_predictors) > 0) {
    ratio_cols <- intersect(ratio_cols, config$selected_new_indicator_predictors)
  }
  if (length(ratio_cols) == 0) {
    stop("No selected NewIndicator predictors found in merged data.")
  }

  # Fit the predictability grid using the NewIndicator ratios as predictors.
  ...
  # Build a sorted comparison table ordered by dependent variable, predictor,
  # then specification for easy manual review.
  ...
  # Write one formatted regression table per future dependent variable.
...
run_new_indicator_predictability_stage <- function(config) {
  enriched <- load_enriched_input(config)
  run_new_indicators_predictability(config, enriched)
}
