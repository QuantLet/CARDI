...
firm_returns_config <- function() {
  ...
  find_table7_root <- function(start_dir) {
    current <- normalizePath(start_dir, mustWork = TRUE)
    repeat {
      if (basename(current) == "Table7" &&
          dir.exists(file.path(current, "Data"))) return(current)
      parent <- dirname(current)
      if (identical(parent, current)) {
        stop("Could not locate Table7 root from: ", start_dir)
      }
      current <- parent
    }
  }

  root <- find_table7_root(script_dir)
  module_dir <- root
  ...
  list(
    ...
    paths = list(
      ...
      carbon_rank = file.path(root, "Data", "Processed", "Carbon_Rank.rds"),
      indicators_monthly = file.path(root, "Data", "NewIndicators", "Monthly", "All_Indicators_Monthly.csv"),
