...
compute_daily_log_returns <- function(firm_long) {
  ...
  progress_every <- max(1L, floor(n_stocks / 20L))

  message("[daily_return] Processing ", n_stocks, " firms.")
  for (i in seq_along(stocks)) {
    sid <- stocks[i]
    if (i == 1L || i == n_stocks || i %% progress_every == 0L) {
      message("[daily_return] Firm ", i, "/", n_stocks,
              " (", round(100 * i / n_stocks, 1), "%)")
    }
...
get_month_end_prices <- function(firm_long) {
  ...
  message("[month_end_price] Processing ", n_stocks, " firms.")
  for (i in seq_along(stocks)) {
    sid <- stocks[i]
    if (i == 1L || i == n_stocks || i %% progress_every == 0L) {
      message("[month_end_price] Firm ", i, "/", n_stocks,
              " (", round(100 * i / n_stocks, 1), "%)")
    }
...
compute_monthly_log_returns <- function(month_end_prices) {
  ...
  message("[monthly_return] Processing ", n_stocks, " firms.")
  for (i in seq_along(stocks)) {
    sid <- stocks[i]
    if (i == 1L || i == n_stocks || i %% progress_every == 0L) {
      message("[monthly_return] Firm ", i, "/", n_stocks,
              " (", round(100 * i / n_stocks, 1), "%)")
    }
